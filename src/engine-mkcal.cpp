...
namespace ayatana
{
    namespace indicator
    {
        namespace datetime
        {
            static const QString TAG_ALARM = "x-lomiri-alarm";
            static const QString TAG_DISABLED = "x-lomiri-disabled";
            ...
            public:
                ...
                void addAlarm (KCalendarCore::Alarm::Ptr pAlarm, std::map<std::pair<DateTime, DateTime>, std::map<DateTime, Alarm>> *pAlarms, QDateTime *pTriggerTime, QTimeZone *pTimeZone, KCalendarCore::Incidence::Ptr pIncidence, bool bAlarm)
                {
                    ...
                    */
                    ...
                    if (bAudioEmpty)
                    {
                        ...
                        {
                            const KCalendarCore::Attachment::List lAttachments = pIncidence->attachments ();

                            for (KCalendarCore::Attachment pAttachment : lAttachments)
                            {
                                bool bUri = pAttachment.isUri ();

                                if (bUri)
                                {
                                    sUri = pAttachment.uri ();
                                    bool bEmpty = sUri.isEmpty ();

                                    if (!bEmpty)
                                    {
                                        cAlarm.audio_url = sUri.toStdString ();
                ...
                void getAppointments (const QDateTime *pDateTimeBegin, const QDateTime *pDateTimeEnd, const QTimeZone *pTimezone, std::function<void (const std::vector<Appointment>&)> pOnAppointments)
                {
                    qDebug () << "getting all appointments from " << *pDateTimeBegin << " to " << *pDateTimeEnd;

                    // Init the default timezone
                    QTimeZone cDefaultTimeZone = QTimeZone ();
                    const QByteArray sTimeZone = pTimezone->id ();
                    QTimeZone cTimeZone = timeZoneFromName (sTimeZone, &cDefaultTimeZone);
                    ...
                    for (KCalendarCore::Incidence::Ptr pIncidence : lIncidences)
                    {
                        ...
                        if (bEmpty == false)
                        {
                            ...
                            {
                                Appointment cAppointment = getAppointment (pIncidence, &cTimeZone);
                                Appointment *pAppointment = nullptr;
                                bool bAlarm = cAppointment.is_alarm ();
                                ...
                                if (!bAlarm)
                                {
                                    ...
                                    if (bRecurs)
                                    {
                                        QDateTime cRecurrenceTime = pRecurrence->getNextDateTime (*pDateTimeBegin);
                                        bool bRecurrenceValid = cRecurrenceTime.isValid ();

                                        while (bRecurrenceValid && cRecurrenceTime <= *pDateTimeEnd)
                                        {
                                            Appointment cRecurringAppointment = cAppointment;
                                            cRecurringAppointment.begin = datetimeFromQDateTime (&cRecurrenceTime, &cTimeZone);
                                            lAppointments.push_back (cRecurringAppointment);
                                            cRecurrenceTime = pRecurrence->getNextDateTime (cRecurrenceTime);
                                            bRecurrenceValid = cRecurrenceTime.isValid ();
                                        }
                                    }
                                    else
                                    {
                                    ...
                                    if (bAlarmInteresting)
                                    {
                                        ...
                                        if (nRepeat && cSnoozeTime)
                                        {
                                            ...
                                            for (int nIter = 0; nIter < nRepeat + 1; nIter++)
                                            {
                                                ...
                                                cTriggerTime = cTriggerTime.toTimeZone (cTimeZone);
                                                this->addAlarm (pAlarm, &dAlarms, &cTriggerTime, &cTimeZone, pIncidence, bAlarm);
                                            }
                                        }
                                        else
                                        {
            ...
            private:
                ...
                static QTimeZone timeZoneFromName (const QString sTimeZone, QTimeZone *pDefaultTimeZone)
                {
                    if (sTimeZone == nullptr)
                    {
                        return QTimeZone ();
                    }

                    // Handle the TZID /freeassociation.sourceforge.net/Tzfile/[Location] case
                    QString sId = sTimeZone;
                    sId = sId.remove ("/freeassociation.sourceforge.net/");
                    sId = sId.remove ("Tzfile/");
                    QByteArray sIdData = sId.toUtf8 ();
                    QTimeZone cTimeZone = QTimeZone (sIdData);
                    bool bValid = cTimeZone.isValid ();

                    if (bValid)
                    {
                        if (pDefaultTimeZone)
                        {
                            *pDefaultTimeZone = cTimeZone;
                        }
                    }
                    else
                    {
                        sId = cTimeZone.displayName (QTimeZone::GenericTime);
                        const bool bEmpty = sId.isEmpty();

                        if (bEmpty)
                        {
                            sId = cTimeZone.country ();
                        }

                        sIdData = sId.toUtf8 ();
                        cTimeZone = QTimeZone (sIdData);
                        bValid = cTimeZone.isValid ();

                        if (!bValid)
                        {
                            qWarning () << "Unrecognized TZID: '" << sTimeZone << "'";
                            cTimeZone = QTimeZone::systemTimeZone ();
                        }
                    }
                ...
                bool isIncidenceInteresting (KCalendarCore::Incidence::Ptr pIncidence)
                {
                    ...
                    const QStringList lCategories = pIncidence->categories ();

                    for (QString sCategory: lCategories)
                    {
                        if (sCategory == TAG_DISABLED)
                        {
                            bDisabled = true;
                        }
                    }
                    ...
                    if (!bDisabled)
                    {
                        ...
                        for (KCalendarCore::Attendee cAttendee : lAttendees)
                        {
                            ...
                            if (bMyEmail)
                            {
                                ...
                                bDisabled = (eAttendeeStatus == KCalendarCore::Attendee::PartStat::Declined);
                ...
                static Appointment getAppointment (KCalendarCore::Incidence::Ptr pIncidence, QTimeZone *pTimeZone)
                {
                    ...
                    const QString sColor = pIncidence->color ();
                    cAppointment.color = sColor.toStdString ();

                    qDebug () << " got appointment from " << cAppointment.begin.format ("%F %T %z").c_str () << " to " << cAppointment.end.format ("%F %T %z").c_str () << ": " << cAppointment.summary.c_str ();
            ...
            void EdsEngine::disable_alarm (const Appointment &pAppointment)
            {
                ...
                if (bAlarm)
                {
                    ...
                    for (KCalendarCore::Incidence::Ptr pIncidence : lIncidences)
                    {
                        QStringList lCategories = pIncidence->categories ();
                        bool bContains = lCategories.contains (TAG_DISABLED);

                        if (!bContains)
                        {
                            lCategories.append (TAG_DISABLED);
                            pIncidence->setCategories (lCategories);
                            bChanged = true;
                        }
                    }
